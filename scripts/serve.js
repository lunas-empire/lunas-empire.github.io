...
const types={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.json':'application/json; charset=utf-8','.webmanifest':'application/manifest+json; charset=utf-8','.bin':'application/octet-stream','.svg':'image/svg+xml','.webp':'image/webp','.png':'image/png'};
export function createPublicServer() {
  return createServer(async(req,res)=>{
    ...
    const file=path==='/'?'index.html':['/s1','/s1/','/s1/index.html'].includes(path)?'s1/index.html':['/manifest.webmanifest','/service-worker.js'].includes(path)?path.slice(1):/^\/assets\/(?:[a-z0-9-]+\/)*[a-z0-9-]+\.(js|css|svg|webp|png)$/.test(path)?path.slice(1):adminFile;
