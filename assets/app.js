...
function today() {
  ...
  return `<h1>MEMBER HUB</h1><p class="intro">${escape(longDate(state.date))}</p>${status()}${notice()}${section('focus',`<ul class="priority-list">${cards}</ul>`)}${section('vs',`<h3>${tx(state.weekday)}</h3>${minimum()}${!shown.has('arms')?details(t('bestArms'),arms(state,guide)):''}${!shown.has('save')?details(t('save'),list(guide.save.filter(permitted))):''}${link('vs','details')}`)}${section('season',`<h3>${escape(phaseLabel(state))}</h3>${list(seasonIds)}${link('season',state.seasonDay?'details':'first24')}`)}${section('daily',`${progress('daily',dailyTasks().map(task=>task.id))}${link('daily','checklist')}`)}${section('next',`<h3>${tx('tomorrow')} · ${tx(WEEKDAYS[(state.weekdayIndex+1)%7])}</h3>${nextCards(1)}`)}`;
}
